...
dotnet publish $PROJECT_DIR -c Release -r linux-x64 --self-contained true /p:PublishSingleFile=true /p:PublishTrimmed=true
if ($LASTEXITCODE -ne 0) {
...
& $GCLOUD_PATH compute ssh "${VM_USER}@${VM_NAME}" --zone=us-west1-b --command="sudo rm -rf $REMOTE_APP_DIR/*"

# Dosyaları gönder
& $GCLOUD_PATH compute scp --recurse "$PUBLISH_DIR/*" "${VM_USER}@${VM_NAME}:${REMOTE_APP_DIR}/" --zone=us-west1-b

# 4. Çalıştırma Yetkisi Ver
Write-Host "4. Çalıştırma yetkisi veriliyor..."
& $GCLOUD_PATH compute ssh "${VM_USER}@${VM_NAME}" --zone=us-west1-b --command="chmod +x $REMOTE_APP_DIR/TRONPANELE_CEKME"
